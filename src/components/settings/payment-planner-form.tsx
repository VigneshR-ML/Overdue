...
export function PaymentPlannerForm({ initialEnabled, initialBps }: { initialEnabled: boolean; initialBps: number }) {
  ...
  async function save(e: React.FormEvent) {
    ...
    try {
      ...
      setMessage(res.ok ? "Auto-plan settings saved." : data.error ?? "Couldn't save settings.")
    } catch { setMessage("Network error — try again.") } finally { setSaving(false) }
  }
  return <form onSubmit={save} className="space-y-3"><label className="flex items-start gap-3 rounded-md border border-hairline bg-paper p-3"><input type="checkbox" checked={enabled} onChange={(e) => setEnabled(e.target.checked)} className="mt-1" /><span><span className="block text-sm font-medium text-ink">Send one automatic proposal per request</span><span className="mt-0.5 block text-[12px] leading-relaxed text-muted">When a debtor requests a payment plan, Overdue calculates one proposed resolution amount and emails it once. Reminders remain paused.</span></span></label><label className="block text-[13px] text-muted">Maximum incentive the proposal may use<div className="mt-1 flex max-w-xs items-center gap-2"><Input value={percent} inputMode="decimal" onChange={(e) => setPercent(e.target.value)} aria-label="Maximum incentive percentage" /><span className="text-sm text-ink">%</span></div></label><p className="text-[12px] leading-relaxed text-faint">The proposal never goes below the outstanding balance minus this percentage. Set 0% to propose the full outstanding amount.</p><Button type="submit" size="sm" variant="outline" disabled={saving}>{saving ? "Saving…" : "Save auto-plan settings"}</Button>{message ? <p className="font-mono text-[11px] text-moss" role="status">{message}</p> : null}</form>
}
