...
export type OwnerNotification = { id: string; title: string; body: string; href: string | null; read_at: string | null; created_at: string }
...
export function NotificationBell({ initial }: { initial: OwnerNotification[] }) {
  ...
  return <div className="relative">
    ...
    {open ? <div className="absolute right-0 top-11 z-50 w-[min(22rem,calc(100vw-2rem))] overflow-hidden rounded-lg border border-hairline bg-surface shadow-2xl">
      ...
      <div className="max-h-80 overflow-y-auto">{items.length ? items.map((item) => <Link key={item.id} href={item.href || "/invoices"} onClick={() => { void read([item.id]); setOpen(false) }} className="block border-b border-hairline px-3 py-3 last:border-0 hover:bg-paper"><div className="flex gap-2"><span className={`mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full ${item.read_at ? "bg-hairline" : "bg-moss"}`} /><div><p className="text-[13px] font-medium text-ink">{item.title}</p><p className="mt-0.5 text-[12px] leading-snug text-muted">{item.body}</p></div></div></Link>) : <p className="p-4 text-sm text-muted">No workflow updates yet.</p>}</div>
    </div> : null}
  </div>
}
