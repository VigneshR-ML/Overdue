...
export default async function InsightsPage() {
  ...
  const fastCash = snap.byCurrency
    ...
    .slice(0, 6)
  const recoveryReportLines = snap.byCurrency.length === 0
    ? ["No open invoice data yet."]
    : snap.byCurrency.map((group) => {
      const outstanding = group.aging.current + group.aging.b0_30 + group.aging.b31_60 + group.aging.b61_90 + group.aging.b90
      const dso = group.dso.days > 0 ? `DSO ${Math.round(group.dso.days)} days` : "DSO needs paid invoice history"
      return `${group.currency}: ${formatMoney(outstanding, group.currency)} outstanding · ${group.aging.pctOverdue}% past due · ${dso}`
    })
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col justify-between gap-4 rounded-lg border border-hairline bg-surface p-4 shadow-ledger sm:flex-row sm:items-center">
        <div>
          ...
          <p className="mt-1 text-sm text-ink-soft">Copy or share a live, currency-by-currency summary from your ledger.</p>
        </div>
        <RecoveryReportActions reportLines={recoveryReportLines} />
      </div>
